...
namespace createManualImports
{
    class Program
    {
        static JObject getCreateImportJson()
        {
        ...
        static JObject getBindingsJson()
        {
            // Remplissez les informations obligatoires
            String nameBindings = "Nom du Binding"; // Nom du binding ('null' pour rien)


            JObject firstColumn = new JObject();
            firstColumn.Add("columnIndex", 0); //Numeros de la colonne
            firstColumn.Add("fieldId", 1234); //Id du field (-10 = On ne prend pas en compte cette colonne)

            JObject secondColumn = new JObject();
            secondColumn.Add("columnIndex", 1); //Numeros de la colonne
            secondColumn.Add("fieldId", 1234); //Id du field (-10 = On ne prend pas en compte cette colonne)


            JObject thirdColumn = new JObject();
            thirdColumn.Add("columnIndex", 2); //Numeros de la colonne
            thirdColumn.Add("fieldId", -10); //Id du field (-10 = On ne prend pas en compte cette colonne)

            JArray binds = new JArray(); //On indique manuellement les colonnes avec les idFields
            binds.Add(firstColumn);
            binds.Add(secondColumn);
            binds.Add(thirdColumn);

            JObject bindingJson = new JObject();
            bindingJson.Add("name", nameBindings); //Nom de ce bindings
            bindingJson.Add("separator", 59); //Code ascii de votre separateur (59 = ;)
            bindingJson.Add("startAt", 2); //Commence a la ligne indique
            bindingJson.Add("binds", binds);
            bindingJson.Add("savedImportFormat", true); //On sauvegarde le format de l'import
        ...
        static JObject getExecutionImportJson(int idBindings)
        {
            // Remplissez les informations obligatoires
            ...
            JObject executionImportData = new JObject();
            executionImportData.Add("binding", idBindings);
            executionImportData.Add("features", features);
        ...
        static void Main(string[] args)
        {
            ...
            Console.Write("SOURCE IMPORT\n" + url + "\n");
            result = sendFile(con, url, xKey, getFileSourceInfo(), "PUT");
            if ((verification(result)) == false)
                System.Environment.Exit(0);
            ...
            Console.Write("CREATION BINDINGS\n");
            result = allConnection(con, url, xKey, getBindingsJson(), "POST");
            if ((verification(result)) == false)
                System.Environment.Exit(0);
            int idBindings = ((dynamic)((dynamic)(JsonConvert.DeserializeObject((String)result[3]))).identifier).guid;
            Console.Write("idBindings = " + idBindings + "\n");

            //Nouvelle url
            url = urlBase + "imports/" + importId + "/executions";
            //On execute l'import
            Console.Write("EXECUTION IMPORT\n");
            result = allConnection(con, url, xKey, getExecutionImportJson(idBindings), "POST");
            if ((verification(result)) == false)
                System.Environment.Exit(0);
